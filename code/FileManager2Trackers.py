...
# Null / False / True
    def is_changed(self, write=True):
        ...
        for a in self.filetracks:
            ...
            if state is None:
                if write:
                    self.cbc('Error: file \'' + a.get_name() + '\' is not accesible\n')
    ...
    def update(self):
        ...
        if changed:
            ...
            if self.needs_vtkfiles:
                ...
                self.vtkfile = 'tempfile_' + str(self.num) + '.vtk'
...
# private
    def recalculate(self):
        if self.vtkfile is not None: #Si no existe VTK no se recalcula
            ...
            if res is not True:
                self.cbc('Error loading file \''+the_file+'\': '+ str(res) +'\n')
                return False
            else:
                self.dim = rr.get_dim()
                ...
                for k, v in rr.get_refs().items():
                    if v is not None:
                        ...
                        templ = list(temps)
                        templ.sort()
                        self.refs[k] = list(map(str, templ)) #For example, self.refs['element_ref'] = [1,2], i.e., list of unique element references

                        self.cbc('Calculating submesh ... ')

                        res = rr.calculate_submesh()

                if not res:
                    self.cbc('Error calculating submesh: \''+the_file+'\'\n')
                    return False

                self.cbc('\n')

                self.cbc('Converting to .vtk ... ')
                ...
                array = list(range(1, rr.nel + 1)) # elementos originales numerados
                ...
                res = k.check()
                if res is not True:
                    self.cbc('Error checking .vtk: k.check(): ' + res + '\n')
                    return False

                self.cbc('Saving \'' + self.vtkfile + '\' ... ')

                res = k.save(self.vtkfile)

                if not res:
                    self.cbc('Error saving file: \''+the_file+'\'\n')
                    return False
...
# does not work with .vtk mesh
    def recalculate(self):
        ...
        if res is not True:
            self.cbc('Error loading file \''+the_mesh+'\': '+ str(res) +'\n')
            return False
        else:
            ...
            for k, v in rr.get_refs().items():
                if v is not None:
                    ...
                    templ = list(temps)
                    templ.sort()
                    self.refs[k] = list(map(str, templ))
            ...
            if res is not True:
                self.cbc('Error reading field: \''+the_field+'\': '+str(res)+'\n')
                return False
            ...
            if anum == fnum:
                ...
                self.cbc('Error: number of items of field file \''+the_field+'\' does not match number of items of mesh file \''+the_mesh+'\'\n')
                return False
            ...
            if not res:
                self.cbc('Error saving file: \''+the_mesh+'\' + \''+the_field+'\'\n')
                return False
    ...
    def recalculate(self):
        if self.vtkfile is not None and self.is_changed(): # si no existe VTK no se recalcula
            ...
            if error is not None:
                self.cbc('Error loading file: \''+the_file+':'+error+'\n')
                return False
        
            vtk = unv.to_vtk()
        
            if not isinstance(vtk, FileMrwVTK.FileMrwVTK):
                self.cbc("Error converting to .vtk: " + vtk + '\n')
                return False
        
            # optional
            res = vtk.check()
            if res is not True:
                self.cbc('Error checking .vtk: vtk.check(): ' + res + '\n')
                return False
    ...
    def combinate(self):
        ...
        for var,data,tracker in zip(self.variables, self.data['formula_data'], self.trackers):
            ...
            if type2 == 'menu-field':
                ...
                if len(texts) != 1:
                    return 'Error reading value: 1 value needed; ' + str(len(texts)) + ' values found.' # admitir vectores / complexos ?
                text = texts[0]
                try:
                    floattext = float(text)
                except ValueError:
                    return "Error converting '" + text + "' to float"
                if Debug: print('Variable ' + name + ': ' + str(floattext))
...
#                print 'Variable ' + name + ': ' + str(floattext)
#                varias.append((name,'value',floattext))
            else:
                return 'Error: variable mode not allowed: ' + str(type2)
    ...
    def check_valid(self, srcs, srcsnames, srcsfieldnames):
        ...
        for s,n,fieldname in zip(srcs, srcsnames, srcsfieldnames):
            ...
            if Debug: print('Variable ' + n + ': size ' + str(size) + ' comp ' + str(comp))
            ...
            if the_size is None:
                ...
                if the_size != size:
                    return 'Variables ' + the_name + ' and ' + n + ': field sizes do not match: ' + str(the_size) + ' and ' + str(size)
    ...
    def get_object(self, name, sources, arrays, formula, variables):
        ...
        while i < tsize:
            ...
            if i == 0:
                ...
                if cursize != acsize:
                    return 'Mixed tuple len: ' + str(acsize) + ' and ' + str(cursize)

            if isinstance(res, tuple) or isinstance(res, list):
                for j in range(acsize):
    ...
    def check_variables_types(self):
        for i in range(len(self.variables)):
            ...
            if var[4] != 'menu-value' and var[4] != 'menu-field':
                return 'Error: variable mode not allowed: ' + str(var[4])
        return True
...
#        print vardata
        ...
        for i in range(len(self.variables)):
            ...
            if var[4] == 'menu-value':
                ...
                if len(texts) != 1:
                    ...
                    return 'Error reading value: 1 value needed; ' + str(len(texts)) + ' values found.'
                text = texts[0]
                try:
                    numtext = float(text)
                except ValueError:
                    ...
                    numtext = vals[0]
                if Debug: print('Variable ' + name + ': ' + str(numtext))
    ...
    def parse_complex(self, string,startchar,endchar,lista):
        ...
        if start<end and start != -1 and end != -1:
            ...
            try:
                if not len(list(map(float, complexstr[1:-1].replace(',', ' ').replace('D','E').split()))) == 2:
                    return -1
                complexparts = list(map(float, complexstr[1:-1].replace(',', ' ').replace('D','E').split()))
