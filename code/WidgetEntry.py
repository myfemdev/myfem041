...
class WidgetEntry(Widget.Widget):
    ...
    def parse_complex(self, string,startchar,endchar,lista):
        ...
        if start<end and start != -1 and end != -1:
            ...
            try:
                if complexstr.find(',') == -1 or not len(list(map(float, complexstr[1:-1].replace(',',' ').replace('D','E').split()))) == 2:
                    return -1
    ...
    def save_mem(self):
        logging.debug('widget entry save mem')
    ...
    def update_from_struct(self):
        logging.debug('Update widget entry')
        val = " ".join(self.struct.get_elements())
