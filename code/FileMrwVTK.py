...
class FileMrwVTK(FileMrw.FileMrw):
    ...
    def add_point_data_sequential(self):
        temp = list(range(1, len(self.points)//3 + 1))
        ...
        for pd in self.pdata:
            if pd.get('type') == 'VECTORS':
                if len(pd['data']) != len(self.points):
                    return 'vectors: #pdata!=#points ' + str(len(pd['data'])) + '!=' + str(len(self.points))
            else: # scalars ?
                if 3 * len(pd['data']) != len(self.points):
                    return 'scalars: #pdata!=#points 3*' + str(len(pd['data'])) + '!=' + str(len(self.points))
        for cd in self.cdata:
            if cd.get('type') == 'VECTORS':
                if len(cd['data']) != 3 * len(self.cells):
                    return 'vectors: #cdata!=#cells '  + str(len(cd['data'])) + '!=3*' + str(len(self.cells))
            else: # scalars ?
                if len(cd['data']) != len(self.cells):
                    return '#cdata!=#cells ' + str(len(cd['data'])) + '!=' + str(len(self.cells))
        if len(self.types) != len(self.cells):
            return '#types!=#cells ' + str(len(self.types)) + '!=' + str(len(self.cells))
        for c in self.cells:
            for p in c:
    ...
    def save2(self, openfile):
        ...
        openfile.write('POINTS ' + str(n1) + ' ' + self.format1 + '\n')
        i = 0
        f = len(self.points)
        while i < f:
        ...
        for cell in self.cells:
            n2 += len(cell) + 1
        openfile.write('CELLS ' + str(n1) + ' ' + str(n2) + '\n')
        for cell in self.cells:
            openfile.write(str(len(cell)))
            openfile.write(' ')
            for v in cell:
                openfile.write(repr(v))
                openfile.write(' ')
        ...
        n1 = len(self.types)
        openfile.write('CELL_TYPES ' + str(n1) + '\n')
        index = 0
        for ctype in self.types:
            openfile.write(str(ctype))
            if index % 10 == 9:
                openfile.write('\n')
            else:
                openfile.write(' ')
        ...
        if len(self.pdata)>0:
            openfile.write('POINT_DATA ' + str(len(self.points)//3) + '\n')
            for d in self.pdata:
                self.write_cell_point_data_vtk(openfile, d)
            openfile.write('\n') # separacion

        if len(self.cdata)>0:
            openfile.write('CELL_DATA ' + str(len(self.cells)) + '\n')
            for d in self.cdata:
                self.write_cell_point_data_vtk(openfile, d)
            openfile.write('\n') # separacion
