...
class WidgetFolderFile(Widget.Widget):
    def __init__(self, ff, parent, window, struct, index):
        ...
        self.header = WidgetHeader.WidgetHeader(self, disp, attribs.get('tooltip'), False, None, struct, window)
    ...
    def event_button(self, event):
        ...
        if attribmeshfield is not None:
            if len(attribmeshfield) == 1:
                if attribs.get(config.AT_SUBTYPE) == 'mesh':
        #            file_extensions = 'All supported (*.mfm;*.unv;*.bdf)|*.mfm;*.unv;*.bdf|Modulef Formatted Meshes (*.mfm)|*.mfm|Universal Files (*.unv)|*.unv|Nastran Input Data File (*.bdf)|*.bdf'
        #            file_extensions = 'All supported (*.mfm;*.unv)|*.mfm;*.unv|Modulef Formatted Meshes (*.mfm)|*.mfm|Universal Files (*.unv)|*.unv'
        #            file_extensions = 'All supported (*.mfm)|*.mfm|Modulef Formatted Meshes (*.mfm)|*.mfm'
                    file_extensions = 'All supported (*.mfm;*.unv;*.vtk;*.vtu;*.pvd)|*.mfm;*.unv;*.vtk;*.vtu;*.pvd|' +\
                ...
                if attribs.get(config.AT_SUBTYPE) == 'field':
        #            file_extensions = 'All supported (*.mff;*.vtu)|*.mff;*.vtu|Modulef Formatted Fields (*.mff)|*.mff|VTK XML Point Data Fields on Unstructured Grid (*.vtu)|*.vtu'
                    file_extensions = 'All supported (*.mff;*.vtk;*.vtu)|*.mff;*.vtk;*.vtu|' +\
                        'Modulef Formatted Fields (*.mff)|*.mff|' +\
                        'VTK Unstructured Grid Fields (*.vtk)|*.vtk|' +\
                        'VTK XML Unstructured Grid Fields (*.vtu)|*.vtu|' +\
                        'All Files (*)|*'

                if attribs.get(config.AT_SUBTYPE) == 'data':
                    file_extensions = 'All Files (*)|*'

            if len(attribmeshfield) == 2:
                if attribmeshfield[0] == 'mesh' or attribmeshfield[0] == 'field' or attribmeshfield[0] == 'data':
    ...
    def parse_mesh_field_string(string):
        ...
        pre = string[:i]
        post = string[i+1:]
        if pre == 'mesh' or pre == 'field' or pre == 'data':
            return [pre, post]
        else:
            return [string]
