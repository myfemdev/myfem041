...
class Source():
        ...
        while i < len(path):
            ...
            if c == '\\':
                if i + 1 < len(path):
                    ...
                    return 'missing char after escape char \'\\\''
            if c == '/':
                ...
                if i + 1 < len(path):
                    if path[i + 1] == '{':
                        if mode == 0:
                            ...
                            return 'only one variable allowed: \'${...}\'. extra \'${\''
    ...
    def desescape(string):
        logging.debug('desescape() is disabled.')
        return string  # Temporary disable of desescape()
        result = ''
        i = 0
        while i < len(string):
            if string[i] == '\\':
