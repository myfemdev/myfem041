...
class WindowStatus(wx.Dialog):
    ...
    def __init__(self, parent, data):
        ...
        if data is None:
            self.load_runstatus()
        elif data.get('queuing') == 'get':
            self.load_runstatus()
            self.data['queuing'] = 'get'
        else:
            self.data = data
        ...
        stt1.SetToolTip(wx.ToolTip('Enter the name or the IP address of the remote machine'))
        ...
        stt2.SetToolTip(wx.ToolTip('Enter your user name in the remote machine'))
        ...
        stt3.SetToolTip(wx.ToolTip('Enter your password when not using keys'))
        ...
        stt4.SetToolTip(wx.ToolTip('Enter or choose your private key file, if used and not automatically detected'))
        ...
        self.stx5.SetToolTip(wx.ToolTip('Choose your private key file if not automatically detected'))
        ...
        #stt6.SetToolTip(wx.ToolTip('Enter a command that will be used to check the job status'))
    ...
    def check_event(self, event):
        ...
        self.stx7.SetBackgroundColour('WHITE')
        self.stx7.ChangeValue('')
    ...
    def load_runstatus(self):
        ...
        try:
            ...
            logging.debug('Problem loading run status data: '+str(repr(why)))
            return
    ...
    def set_state(self, state=None):
        if state is None:
            ...
            state = self.data.get('state')
        if state == 'Finished':
            self.stx7.SetBackgroundColour('GREEN')
        elif state == 'Finished (with error)':
            self.stx7.SetBackgroundColour('RED')
        else:
            self.stx7.SetBackgroundColour('YELLOW')
