...
class PlotNumbering(Plot.Plot):
    ...
    def get_options(self):
        ...
        return {'interactor':True,'title':'Numbering'}
...
# wireframe
    ...
    def update_widget(self, pointnum):
        self.struct.set_elements_nosel([str(pointnum)])
...
#        print 'nums', nums
    ...
    def search_callback_coords(self):
        ...
        if self.data1.get('fielddomain') == 'cell':
            ...
            return {'x':str((bounds[0]+bounds[1])/2.0),
                    'y':str((bounds[2]+bounds[3])/2.0),
                    'z':str((bounds[4]+bounds[5])/2.0)}
        else:
            o = self.src.GetOutput()
