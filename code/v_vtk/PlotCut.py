...
# mostra parte do obxecto. extrae partes de celdas
...
class PlotCut(Plot.Plot):
    ...
    def get_options(self):
        ...
        return {'interactor':True,'title':'Cut'}
...
# write params to menu xml structure on first load. only writes if there is no data
    def set_params(self, struct, origin, normals, force=False):
        ...
        if len(ch) < 2:
            print('Error: incorrect number of childs in PlotCut')
            return
    ...
    def copy_params(self, struct):
        ...
        result = self.read_2_vectors(struct, [' in PlotCut',' in origin',' in normal'])
