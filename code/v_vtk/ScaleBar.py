...
class ScaleBar(wx.Panel):
    ...
    def __init__(self, parent, plot):
        ...
        b.SetToolTip(wx.ToolTip('Toogle visibility of scale'))
        self.buttons.append(b)

        self.color_pos = 0
        self.color_choices = ['Blue to red', 'Red to Blue','Cool','Warm']
        ...
        self.color_widget.SetToolTip(wx.ToolTip('Change scalarbar color range'))
        self.buttons.append(self.color_widget)
        
        self.abs_rel_pos = 0
        self.abs_rel_choices = ['Absolute scale', 'Relative scale']
        ...
        self.abs_rel_widget.SetToolTip(wx.ToolTip('Toogle scale relative to visible part'))
        self.buttons.append(self.abs_rel_widget)

        self.mode_code = ''
        self.mode_choices = ['Adjust manually', 'Adjust to the ' + self.data1.get('evolution_lower') + ' interval', 'Adjust to the current field', 'Adjust to every field']
        ...
        self.mode_choice.SetToolTip(wx.ToolTip('Choose scale range'))
...
#            self.mode_handler()
    ...
    def color_event(self, event):   
        ...
        if self.color_pos == 0: # blue->red
            self.plot.scalarbar_change_color((0.66667, 0.0), (1.0, 1.0), (1.0, 1.0), 'Curve')
        elif self.color_pos == 1: # red->blue
            self.plot.scalarbar_change_color((0.0, 0.66667), (1.0, 1.0), (1.0, 1.0), 'Curve')
        elif self.color_pos == 2: # cool
            self.plot.scalarbar_change_color((0.66667, 0.5), (1.0, 0.0), (0.7, 1.0), 'Sqrt')
        elif self.color_pos == 3: # warm
            self.plot.scalarbar_change_color((0.16667, 0.0), (0.0, 1.0), (1.0, 0.7), 'Sqrt')
#        self.plot.do_render()
    ...
    def update_range2(self, range):
        if range is None:
            ...
            self.txt_r0.ChangeValue(str(range[0]))
            self.txt_r1.ChangeValue(str(range[1]))
    ...
    def time_range_step(self, current, total):
        ...
        if current is None and self.progress is not None:
            ...
            self.progress = wx.ProgressDialog('Reading files','Read 0 of '+str(total)+' files', total, \
                self.plot.window, wx.PD_CAN_ABORT | wx.PD_AUTO_HIDE | wx.PD_APP_MODAL)
        elif self.progress is not None:
            res = self.progress.Update(current,'Read '+str(current)+' of '+str(total)+' files')
