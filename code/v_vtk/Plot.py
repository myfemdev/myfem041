...
import Menus
import auxSave
from v_vtk import MainBar 
from v_vtk import ScaleBar
from v_vtk import TimeBar 
from v_vtk import ScalarRange
from v_vtk import TimeManager2
import Player
import PlayerConst
import v_vtk.configPlot  as configPlot
...
#        wx.Frame.__init__(self, parents[0]) # parents[0] is the book
        ...
        if self.get_options().get('split') is True: # excepcional
        ...
        elif self.get_options().get('interactor') is True:
...
# <data1 tracker src>
    ...
    def call_data1(self, struct):
        ...
        if isinstance(self.data1,str):
            self.window.errormsg('Error creating plot: '+self.data1)
            return False
...
#        print 'Plot.tracker: vtk_filename', self.filename, 'revision', self.revision, 'result', result
        if result is None:
            self.window.errormsg('Error converting mesh to vtk. Not creating plot')
            return False
    ...
    def call_src(self):
        ...
        if fd == 'point':
            ...
            self.window.errormsg('Incorrect field domain: '+str(fd))
            return False
        ...
        if ft == 'scalar':
            ...
            self.window.errormsg('Incorrect field type: '+str(ft))
            return False
    ...
    def update_tracker_source(self, index=None): # que fai para os pvds: nada: self.has_src é false
        ...
        if self.additional:
            ...
            if src is None or isinstance(src, str):
                return 'Error obtaining VTK source object (main): ' + str(src)
            src2 = self.tracker.get_src_group_f(2, self.fielddata, mnames,index) # pode ser None: non ten
            if src2 is None or isinstance(src2, str):
                return 'Error obtaining VTK source object (additional): ' + str(src2)
        else:
            src = self.tracker.get_src(mnames)
            if src is None or isinstance(src, str):
                return 'Error obtaining VTK source object: ' + str(src)
...
# </data1 tracker src>
    ...
    def do_render(self):
        ...
        self.widget.Render()
        if self.get_options().get('split') is True:
            self.widget2.Render()
        ...
        if changes.get('new'):
            ...
            if self.has_assign:
                if self.tracker.is_nodepvd:
                    ...
                    if src is None or isinstance(src, str):
                        return 'Error obtaining VTK source object (additional): ' + str(src)
                    self.src2.SetInputConnection(src2.GetOutputPort())
                    self.src2.Update()
                else:
                    self.src.SetInputConnection(src.GetOutputPort())
...
# <scale>
    def add_scale_1(self):
        self.scale_button = wx.ToggleButton(self.plotbar, wx.ID_ANY, 'Scale bar', style=wx.BU_EXACTFIT)
...
# # # # <labels b>
    ...
    def add_labels_b_1(self):
        labels_b_choices = ['No labels', 'Selected', 'All']
...
# # # # surface / wireframe
    def add_sw_1(self, button=True, selection=1):
        ...
        self.sw_choices = ['Wireframe', 'Surface']
        if self.sw_button:
...
# # # # wireframe / surface / surface+edges
    def add_swe_1(self, selection=1):
        ...
        self.swe_choices = ['Wireframe', 'Surface', 'Surface+Edges']
...
# # # # <save image as>
    ...
    def save_as_call(self, event,movie=False):
        ...
        if movie:
            ...
            if self.get_options().get('split') is True: # excepcional
                self.save_from_window(self.widget.GetRenderWindow(), 'Select a file to save the scene on the left')
                self.save_from_window(self.widget2.GetRenderWindow(), 'Select a file to save the scene on the right')
            else:
...
#                        temp = str(f.encode('string_escape')) # solucion alternativa a NFKD-ignore máis abaixo
#                    txt += 'File: ' + temp + '\n'
        if dim is not None:
            txt += 'Dim: ' + str(dim) + 'D\n'
        ...
        if time is not None:
            txt += evolution + ': ' + str(time) + '\n'

        txt2 = txt #unicodedata.normalize('NFKD', txt).encode('ascii','ignore') # cuestionable º -> o
        if txt != txt2:
            #print 'Warning: unicode to ascii has changed some chars : \'' + txt + '\' -> \'' + txt2 + '\'' #code prior version 0.0.1
            logging.warning('Warning: unicode to ascii has changed some chars : \''+str(txt)+'\' -> \''+str(txt2)+'\'')
...
# # # # Opacity: 100%/Opacity: 75%/Opacity: 50%/Opacity: 25%/Opacity: 0%
    def add_opacity_1(self, selection=1):
        ...
        self.opacity_choices = ['Opacity: 100%', 'Opacity: 90%', 'Opacity: 80%', 'Opacity: 70%',\
'Opacity: 60%', 'Opacity: 50%', 'Opacity: 40%', 'Opacity: 30%', 'Opacity: 20%', 'Opacity: 10%']
...
# avoid crash when opening closing exiting (wx/vtk BUG) - does not solve it
    ...
    def printcam(self):
        ...
        #print 'gv', ren.GetActiveCamera().GetViewUp() #code prior version 0.0.1
...
# establecemos el escritor de video
    def set_movie_writer(self, codec,filename,rate):
        ...
        self.moviewriter = None
        codecs = ['AVI','FFMPEGHQ','FFMPEGLQ','OGGTHEORA','MPEG2']
        ...
        if codec in codecs:
            try:
                ...
                return 'ERROR: '+codec+ ' codec not found'
        else:
            return 'ERROR: '+codec+ ' not yet supported'
...
# Exportar animación de la ventana de render a video
    def start_movie_saving(self, movie_opt=None):
        if movie_opt is not None:
            ...
            ini_time_index = self.tracker.search_time_pos(movie_opt.get('start_time'))
            end_time_index = self.tracker.search_time_pos(movie_opt.get('end_time'))
            movie_opt['num_times'] = end_time_index - ini_time_index
            rate = movie_opt.get('num_times')//movie_opt.get('duration')
            if rate == 0:
                rate = 1
            #Creamos movie writer e image filter
            res =  self.set_movie_writer(movie_opt.get('codec'),movie_opt.get('file'),rate)
            if isinstance(res,str):
                self.window.errormsg(res)
                return False
            else:
                self.set_image_filter(self.moviewriter)
                self.movie_saving = movie_opt.get('movie')
...
# We import PlotMesh here because:
# - Plot uses PlotMesh to return the value of get_type_type() and build(), so it ONLY has to know PlotMesh class, 
#   so a final "import PlotMesh" is possible.
# - PlotMesh uses Plot to define "class PlotMesh(Plot.Plot)"; therefore, class Plot must be defined BEFORE being imported in PlotMesh, so 
#   an initial "Import PlotMesh" is not possible.
# 
# Note 1: If class PlotMesh() would not inherit from Plot.Plot, an initial "import PlotMesh" would be possible. 
# See https://stackoverflow.com/questions/7336802/how-to-avoid-circular-imports-in-python for the following valid example:
# ***** a.py:
#import b
#class A:
#    def bar(self):
...
#        return a.A()
#
# Note2: the fact that get_type_type() and build() are static method does not affect here
from v_vtk import PlotMesh
from v_vtk import PlotReferences
from v_vtk import PlotNumbering
from v_vtk import PlotMaterials
from v_vtk import PlotFilled
from v_vtk import PlotThreshold
from v_vtk import PlotContour
from v_vtk import PlotScalarDeformed
from v_vtk import PlotVectorDeformed
from v_vtk import PlotOverLine
from v_vtk import PlotSlice
from v_vtk import PlotCut
from v_vtk import PlotRoughCut
from v_vtk import PlotVectorField
from v_vtk import Plot2DGraph
from v_vtk import PlotVectorComponents
from v_vtk import PlotStreamline
from v_vtk import PlotPicture
