...
#        self.vali = " ".join(struct.get_elements())
        ...
        if res[0] == 1 or res[0] == 2:
            ...
            self.filename = None
            unused = self.check_error ('Error parsing value of showfile attribute')
        if self.filename is not None:
            ...
            try:
                ...
                self.text = f.read()
                f.close()
            except IOError as x: # notificar o usuario
                ben = False
                print('WidgetText:', repr(x))
            if not ben:
                unused = self.check_error ('Error reading file: \'' + self.filename + '\'')
        elif res[0] == 1 or res[0] == 2:
            unused = self.check_error ('No file found')
...
#    def save_log(self):
#        val = ' '.join(self.entry.GetValue().split())
#        if val != self.vali:
#            self.log(val)
