...
class WindowTabular(wx.Frame):
    ...
    def __init__(self, parent, onclose=None):
        ...
        wx.Frame.__init__(self, parent, wx.ID_ANY, 'Tabular data', style=wx.DEFAULT_FRAME_STYLE|wx.FRAME_FLOAT_ON_PARENT, size=(400,400))
    ...
    def add_grid(self,rowlabels = False):
        ...
        self.grid.Bind(wx.grid.EVT_GRID_CELL_CHANGED, self.on_grid_cell_change)
        if not rowlabels: 
...
#        self.grid.SetRowMinimalAcceptableHeigth(self.grid.GetRowSize(0))
    ...
    def build_from_file(self,new):
        ...
        parsed = self.struct.parse_source_string_1(self.struct.get_attribs().get('data'))
        #data="file:..."
        if parsed[0] == 1: 
            ...
            if len(sourcemenu) > 0:
                ...
                self.parent.errormsg( 'Can not parse "file:" string value or path does not exist')
                self.Close()
                return False
        #data="menu:..."
        elif parsed[0] == 2: 
            ...
            if len(sourcemenu) > 0:
                ...
                self.parent.errormsg( 'Can not parse "menu:" string value or path does not exist')
                self.Close()
                return False
        else: 
            self.parent.errormsg( 'Can not parse "data" string value: only allows "menu:" and "file:" prefix')
            self.Close()
            return False
        ...
        self.ncols = matrix.get('col_num')
        self.nrows = matrix.get('row_num')
...
#        self.grid.SetRowMinimalAcceptableHeigth(self.grid.GetRowSize(0))
    ...
    def read_matrix(self,filename):
        ...
        err = f.open(filename)
        if err is not True:
            self.parent.errormsg ('Error opening file: ' + str(err))
            self.Close()
            return False
        ...
        temp = f.getword()
        if temp is None:
            self.parent.errormsg ('Premature EOF')
            return False
        if temp is False:
            self.parent.errormsg ('I/O Error: ' + f.get_error())
            return False

        matrix['row_num'] = FileParse.FileParse.to_int(temp)
        if matrix.get('row_num') is False:
            self.parent.errormsg ('Error converting \'' + str(temp) + '\' to integer 1')
            return False
        ...
        if temp is None:
            ...
            return False
        if temp is False:
            self.window.errormsg ('I/O Error: ' + f.get_error())
            return False

        matrix['col_num'] = FileParse.FileParse.to_int(temp)
        if matrix.get('col_num') is False:
            self.parent.errormsg ('Error converting \'' + str(temp) + '\' to integer')
            return False
        matrix['row_list'] = []

        #Lectura de datos por filas
        for s in range(matrix.get('row_num')):

            row=[]
            for r in range(matrix.get('col_num')):
                temp = f.getword()
                if temp is None:
                    self.parent.errormsg ('Premature EOF')
                    return False
                if temp is False:
                    ...
                    return False
                row.append(temp)
            matrix.get('row_list').append(row)

        #Lectura de las etiquetas de las filas
        matrix['row_labels'] = [] # string o None
        for t in range(matrix.get('row_num')):
            temp = f.getline()
            if temp is False:
                self.window.errormsg ('I/O Error: ' + f.get_error())
                return False
            ...
            matrix.get('row_labels').append(temp)

        #Lectura de las etiquetas de las columnas
        matrix['col_labels'] = [] # string o None
        for t in range(matrix.get('col_num')):
            temp = f.getline()
            if temp is False:
                self.window.errormsg ('I/O Error: ' + f.get_error())
                return False
                ...
                temp = temp.rstrip() # quita \n e espazos
            matrix.get('col_labels').append(temp)

        err = f.close()
        if err is not True:
            self.window.errormsg ('Error closing file: ' + str(err))
            return False
        f = None
