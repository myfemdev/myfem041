...
# modifies self even if read is unsuccessfull
    ...
    def read2(self, data):
        ...
        if t is False:
            return 'ValueError (nel)'
        self.nel = t[0]

        t = FileMrw.FileMrw.read_ints(data, 1, 1)
        if t is False:
            return 'ValueError (nnod)'
        self.nnod = t[0]

        t = FileMrw.FileMrw.read_ints(data, 2, 1)
        if t is False:
            return 'ValueError (nver)'
        self.nver = t[0]

        t = FileMrw.FileMrw.read_ints(data, 3, 1)
        if t is False:
            return 'ValueError (dim)'
        self.dim = t[0]

        t = FileMrw.FileMrw.read_ints(data, 4, 1)
        if t is False:
            return 'ValueError (lnn)'
        self.lnn = t[0]

        t = FileMrw.FileMrw.read_ints(data, 5, 1)
        if t is False:
            return 'ValueError (lnv)'
        self.lnv = t[0]

        t = FileMrw.FileMrw.read_ints(data, 6, 1)
        if t is False:
            return 'ValueError (lne)'
        self.lne = t[0]

        t = FileMrw.FileMrw.read_ints(data, 7, 1)
        if t is False:
            return 'ValueError (lnf)'
        self.lnf = t[0]

        self.cc( \
            'nel ' + str(self.nel) + ' nnod ' + str(self.nnod) + ' nver ' + str(self.nver) + \
            ' dim ' + str(self.dim) + ' lnn ' + str(self.lnn) + ' lnv ' + str(self.lnv) + \
            ' lne ' + str(self.lne) + ' lnf ' + str(self.lnf) + \
            '\n')

        if askdim is not None and self.dim != askdim:
            return 'Dimensions do not match: ' + str(self.dim) + '!=' + str(askdim)
...
#        if count != expected:
#            return 'Incorrect number of words ( counted ' + str(count) + ' != expected ' + str(expected) + ' )'

        n = self.firstline

        self.cc('nn ' + str(num_nn) + ' ')
        q = FileMrw.FileMrw.read_ints_1(data, n, num_nn) # _1 => -1
        if q is False:
            return 'ValueError (nn)'
        self.nn = q
        n += num_nn

        self.cc('mm ' + str(num_mm) + ' ')
        q = FileMrw.FileMrw.read_ints_1(data, n, num_mm) # _1 => -1
        if q is False:
            return 'ValueError (mm)'
        self.mm = q
        n += num_mm

        self.cc('nrc ' + str(num_nrc) + ' ')
        q = FileMrw.FileMrw.read_ints(data, n, num_nrc)
        if q is False:
            return 'ValueError (nrc)'
        self.nrc = q
        n += num_nrc
                
        self.cc('nra ' + str(num_nra) + ' ')
        q = FileMrw.FileMrw.read_ints(data, n, num_nra)
        if q is False:
            return 'ValueError (nra)'
        self.nra = q
        n += num_nra

        self.cc('nrv ' + str(num_nrv) + ' ')
        q = FileMrw.FileMrw.read_ints(data, n, num_nrv)
        if q is False:
            return 'ValueError (nrv)'
        self.nrv = q
        n += num_nrv
        ...
        self.cc('nsd ' + str(num_nsd) + ' ')
        q = FileMrw.FileMrw.read_ints(data, n, num_nsd)
        if q is False:
            return 'ValueError (nsd)'
        self.nsd = q
        n += num_nsd
                
#        if n != count:
#            return 'Incorrect number of read words ( read ' + str(n) + ' != counted ' + str(count) + ' )'
        ...
        if verbose:
            self.cc('set element_ref # ' + str(sorted(list(self.sets['element_ref']))) + '\n')
        else:
            self.cc('set element_ref # ' + str(len(self.sets['element_ref'])) + ' ')

        if self.dim >= 3:
            #self.cc('set nrc ')
            self.sets['face_ref'] = set(self.nrc)
            if verbose:
                self.cc('set face_ref # ' + str(sorted(list(self.sets['face_ref']))) + '\n')
            else:
                self.cc('set face_ref # ' + str(len(self.sets['face_ref'])) + ' ')

        if self.dim >= 2:
            #self.cc('set nra ')
            self.sets['edge_ref'] = set(self.nra)
            if verbose:
                self.cc('set edge_ref # ' + str(sorted(list(self.sets['edge_ref']))) + '\n')
            else:
                self.cc('set edge_ref # ' + str(len(self.sets['edge_ref'])) + ' ')

        if self.dim >= 1:
            #self.cc('set nrv ')
            self.sets['vertex_ref'] = set(self.nrv)
            if verbose:
                self.cc('set vertex_ref # ' + str(sorted(list(self.sets['vertex_ref']))) + '\n')
            else:
                self.cc('set vertex_ref # ' + str(len(self.sets['vertex_ref'])) + ' ')
...
#            smr = []
 ...
 #           self.smn.append('nrv')
            ...
            self.submeshed = True
            self.cc('generated ' + str(self.nsm) + ' submesh cells')
...
# creates a FileVTK object
    def to_vtk_vtu(self, mode):
        ...
        if self.submeshed:
            string = str(self.nel) + '+' + str(self.nsm) + ' cells'
        else:
            string = str(self.nel) + ' cells'

        vtk.set_comm('Data ' + str(self.dim) + 'D ' + string)
