...
#        self.image = wx.Image(os.path.join(self.path,os.pardir,config.DIR_IMAGES,'background.jpg'))
        if os.path.isfile(os.path.join(self.path,os.pardir,config.DIR_IMAGES,'background.'+window.title+'.png')):
            self.image = wx.Image(os.path.join(self.path,os.pardir,config.DIR_IMAGES,'background.'+window.title+'.png'))
        else:
            self.image = wx.Image(os.path.join(self.path,os.pardir,config.DIR_IMAGES,'background.opennum.png'))
        if not self.image.IsOk(): # fallback
            self.image = wx.EmptyImage(1,1)
        #self.bitmap = wx.BitmapFromImage(self.image) #old wx
...
# rethink: add (plot=!="true") != update (plot ="true")
        ...
        if isinstance(data, str):
            self.window.errormsg('Error creating/updating plot: ' + data)
            return False
        ...
        extra = '¬' # para abrir, en distintas graficas, structs con igual ficheiro pero distinta configuracion de tempo/frecuencia...
        if not data.get('interpolation'):
            extra += str(data.get('interpolation'))
        extra += '¬'
...
#        print('plotlist was', was is not None, filenames, fieldname, typename, extra, struct
        ...
        if add:
            #self.window.add_text('Adding visualization start ...\n') #code prior version 0.0.1
            logging.debug('Adding visualization start ...')
            tracker = struct.get_tracker5( filemanager, data )
            if isinstance(tracker, str):
                self.window.errormsg('Error creating plot: ' + tracker)
            elif tracker is None:
                self.window.errormsg('Error creating plot: Undefined plot files')
            # si añadimos un nuevo tracker vacio
            elif tracker.is_void:
                self.window.errormsg('Error creating plot: No mesh or chart has been selected')
            else:
...
#                    print(filenames, fieldname, typename, extra, struct, tracker, data
                    self.window.errormsg('Unable to access file(s) to create plot')

            #self.window.add_text('Adding visualization end ...\n') #code prior version 0.0.1
            logging.debug('Adding visualization end ...')
            
        if update:
        
            #print('Updating visualization start ...' #code prior version 0.0.1
            logging.debug('Updating visualization start ...')

            tracker = struct.get_tracker5( filemanager, data ) 

            if isinstance(tracker, str):
                self.window.errormsg('Error updating plot: ' + tracker)
            elif tracker is None:
                self.window.errormsg('Error updating plot: Undefined plot files')
            else:
                ...
                if tracker.is_changed() is not None:
                    if tracker.update() is not None:
                        if view.updated_tracker(tracker):
                            if view.updatable_tracker():
                                ...
                                if isinstance(res, str):
                                    self.window.errormsg('Error updating plot with new data files: ' + res)
                        ...
                        self.window.errormsg('Error converting mesh to vtk. Not updating plot')
                else: # tracker.is_changed() is None
                    self.window.errormsg('Unable to access file(s) to update plot')
            ...
            #print('Updating visualization end ...' #code prior version 0.0.1
            logging.debug('Updating visualization end ...')

        if was is not None and not add and mode != 6:
        
            #print('Selecting visualization start ...' #code prior version 0.0.1
            logging.debug('Selecting visualization start ...')
            next = self.book.GetPageIndex(view)
            previous = self.book.GetSelection()
            if next != previous:
                self.book.SetSelection(next)
            #print('Selecting visualization end ...' #code prior version 0.0.1
            logging.debug('Selecting visualization end ...')
