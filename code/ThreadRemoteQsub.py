...
class ThreadRemoteQsub(threading.Thread):
    ...
    def __init__(self, parent, window, options):
        ...
        if not isinstance(self.appname,str) or len(self.appname) < 1: # non debera pasar
            self.appname = 'void'
        ...
        self.dirfiles = 'input' # input file folder
        self.direxec = 'output' # output file folder
    ...
    def exec_solver2(self):
        for executable in self.options.get('executables'):
            ...
            self.options['state'] = 'Submitted'
            self.options['exec_command'] = temp
            self.options['local_dir'] = os.path.abspath(str(os.getcwd(), sys.getfilesystemencoding()))
    ...
    def check_status(self):
        ...
        if out[0] is not None and out[0] != '':
            self.options['state'] = 'Not finished'
        else:
            ...
            for line in out:
                ...
                if parts[0].strip() == 'exit_status':
                    ...
                    if parts[1].strip() == '0':
                        self.options['state'] = 'Finished'
                        return True
                    else:
                        self.options['state'] = 'Finished (with error)'
                        return True
        return True
