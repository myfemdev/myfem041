...
subroutine leema(iformat)
  ...
  use fich_electros, only : fichma,fichteta
  use malla_2DP1
  use electros_2D,   only : mua,b,sol,iopteta,teta
 
  implicit none
  integer, intent(in) :: iformat
  integer             :: ierror,nnod,nverteta
  integer             :: i,j,k
  ...
  select case(iformat)
    ...
    case(1)
      ...
      rewind(10)
      read(unit=10,fmt=*) nel,nnod,nver
      ...
      read(unit=10,fmt=*)((mm(j,k),j=1,3),k=1,nel),((nra(j,k),j=1,3),k=1,nel),&
                         ((nrv(j,k),j=1,3),k=1,nel),((z(j,i),j=1,2),i=1,nver)
      read(unit=10,fmt=*)(nsd(k),k=1,nel)
      close(unit=10)
    ...
    case(2)
      ...
      read(unit=10)((mm(j,k),j=1,3),k=1,nel),((nra(j,k),j=1,3),k=1,nel),&
                         ((nrv(j,k),j=1,3),k=1,nel),((z(j,i),j=1,2),i=1,nver)
      read(unit=10)(nsd(k),k=1,nel)
      close(unit=10)
