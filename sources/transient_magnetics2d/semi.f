...
C              CALCULO DE LA INTEGRAL MEDIANTE LA FORMULA DEL BARICENTRO
            ...
            ybar = (z(2,mm1) + z(2,mm2) + z(2,mm3)) / 3.D0
      		...
      		xint =f(time,xbar,ybar,nsd(k),i,sourcevol%itipo(i),sourcevol%modo(i),
...
C              INTEGRAL EN EL TRIANGULO K (FORMULA DE LOS VERTICES)
      	    ...
      	    f1=f(time,z(1,mm1),z(2,mm1),nsd(k),i,sourcevol%itipo(i),
     &       sourcevol%modo(i),sourcevol%valor(i),sourcevol%etiqueta(i))
           	f2=f(time,z(1,mm2),z(2,mm2),nsd(k),i,sourcevol%itipo(i),
     &       sourcevol%modo(i),sourcevol%valor(i),sourcevol%etiqueta(i))
           	f3=f(time,z(1,mm3),z(2,mm3),nsd(k),i,sourcevol%itipo(i),
     &       sourcevol%modo(i),sourcevol%valor(i),sourcevol%etiqueta(i))
...
C              INTEGRAL EN EL TRIANGULO K (FORMULA DE LOS PUNTOS MEDIOS)
             ...
             xint12 = f(time,x12,y12,nsd(k),i,sourcevol%itipo(i),
     &         sourcevol%modo(i),sourcevol%valor(i),
     &         sourcevol%etiqueta(i)) * det 
             xint23 = f(time,x23,y23,nsd(k),i,sourcevol%itipo(i),
     &         sourcevol%modo(i),sourcevol%valor(i),
     &         sourcevol%etiqueta(i)) * det 
             xint31 = f(time,x31,y31,nsd(k),i,sourcevol%itipo(i),
     &         sourcevol%modo(i),sourcevol%valor(i),
     &         sourcevol%etiqueta(i)) * det 
...
C                       LONGITUD DEL INTERVALO
     ...
     & (gs(time,x1,y1,iref,isu,sourcesur%itipo(isu),sourcesur%modo(isu),
     &   sourcesur%valor(isu),sourcesur%etiqueta(isu))
     &   *xnod2 + 
     &  gs(time,x2,y2,iref,isu,sourcesur%itipo(isu),sourcesur%modo(isu),
     &   sourcesur%valor(isu),sourcesur%etiqueta(isu))
     &   *xnod1)
...
C                       LONGITUD DE LA ARISTA
     ...
     &                (z(2,nov2) - z(2,nov1))**2)
        ...
        b(nov1) = b(nov1) + delta *  0.5d0 * 
     &   (g(time,x1,y1,iref,ine,neumann_bc%modo(ine),
     &   neumann_bc%valor(ine),neumann_bc%etiqueta(ine))
     &   *xnod2 + 
     &   g(time,x2,y2,iref,ine,neumann_bc%modo(ine),
     &   neumann_bc%valor(ine),neumann_bc%etiqueta(ine))
     &   *xnod1)
