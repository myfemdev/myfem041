...
!-----------------------------------------------------------------------
use module_COMPILER_DEPENDANT, only: DOUBLE, MAXPATH
use module_ALLOC
use module_FILES
implicit none
...
! variables filled by the Modulef mesh file
  ...
  real(DOUBLE), dimension(:, :), allocatable :: xv !vertices coordinates array
  real(DOUBLE), dimension(:, :), allocatable :: xd !nodes coordinates array
  integer,      dimension(:),    allocatable :: rl !element index array
end type

!Private procedures
private :: feed, empty, stringd
