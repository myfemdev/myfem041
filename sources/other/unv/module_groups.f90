...
!Private procedures
private :: binary_search, insert_prv, alloc_prv
...
subroutine alloc_prv(v, n)
  ...
  if (allocated(v)) then
    if (size(v,1) == n) return
    deallocate(v, stat = res)
    if (res /= 0) call error('(module_groups/alloc/deallocate) Unable to deallocate variable')
  end if
  allocate(v(n), stat = res) !In f2003 add: errmsg = cad
  if (res /= 0) call error('(module_groups/alloc/allocate) Unable to allocate variable')
...
call enlarge(cell, row=n+1, col=size(v,1), fit_col=.true.)
