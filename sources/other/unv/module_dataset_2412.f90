...
!Private procedures
private :: check_and_set, reorder_nodes, find, swap
...
!-----------------------------------------------------------------------
subroutine read_2412(iu, m, maxdim)

  integer,                               intent(in)    :: iu !unit number for unvfile
  type(mesh), allocatable, dimension(:), intent(inout) :: m  !meshes
...
!   reorder nodes to obtain vertices first and counter-clockwise orientation
    call reorder_nodes(m(size(m,1)), m(d), nn)

!   copy nodes to mesh (add an element after the last one)
    call set(m(d)%id, nn, col=m(d)%nl+1, fit_row=.true.)
    m(d)%nl = m(d)%nl + 1

!   set the new element in fes list of elements
    call set(cells, [d, m(d)%nl], col=Field1, fit_row=.true.)
  end do
...
! REMARK: only valid for triangles (P1 & P2) and tetrahedra (P1 & P2)
!-----------------------------------------------------------------------
subroutine reorder_nodes(mx, m, nn)
type(mesh), intent(in)    :: mx   !mesh with coordinates
type(mesh), intent(in)    :: m    !mesh
integer,    intent(inout) :: nn(:)!nodes
integer :: i, j, l, vf, newnn(10)
real(DOUBLE) :: a2(3), a3(3), a4(3)
...
if (m%LNN /= m%LNV) then
    ...
    do j = 1, m%LNN !first possible vertex
      if (j /= i) then
        do l = j+1, m%LNN !second possible vertex
          if (l /= i) then
            if (  maxval(abs(mx%xd(:,nn(j))-mx%xd(:,nn(l)))) > 1e3*epsilon(mx%xd) .and. & !ensures that is (j,l) not a singular egde
            maxval(abs((mx%xd(:,nn(j))+mx%xd(:,nn(l)))/2-mx%xd(:,nn(i)))) < 1e3*epsilon(mx%xd) ) cycle NODES
          end if
        end do
      end if
    end do
  ...
  !identify midpoints
  do i = 1, m%LNN !nodes
    if (find(newnn == nn(i))>0) cycle !it is a vertex
...
!-----------------------------------------------------------------------
! find: find the first occurrence of a value in a vector
!-----------------------------------------------------------------------
function find(v) result(res)
