...
subroutine dealloc_prv(v)
  ...
  if (res /= 0) call error('(module_ALLOC_real_r1/dealloc) Unable to deallocate variable')
...
subroutine alloc_prv(v, rows)
  ...
  if (res /= 0) call error('(module_alloc_real_r1/alloc) unable to allocate variable')
  v(1:n) = 0.
...
subroutine reduce_prv(v, length)
  ...
  if (.not. allocated(v)) then !v not allocated
    call info('(module_ALLOC_real_r1/reduce) Variable not allocated'); return
  end if
  if (length > size(v, 1)) then !p too large
    call info('(module_ALLOC_real_r1/reduce) Given dimension is too large'); return
  end if    
