module module_ALLOC_real_DOUBLE_r1
!-----------------------------------------------------------------------
! Module for memory allocation of real rank 1 allocatable arrays
! Last update: 28/07/2009real
...
subroutine dealloc_prv(v)
  ...
  if (res /= 0) call error('(module_ALLOC_real_DOUBLE_r1/dealloc) Unable to deallocate variable')
...
subroutine alloc_prv(v, rows)
  ...
  if (res /= 0) call error('(module_alloc_real_DOUBLE_r1/alloc) unable to allocate variable')
  v(1:n) = 0.
...
subroutine extend_prv(v, extension)
  ...
  temp(1:n0) = v
  temp(n0+1:n) = 0.   !reset new positions      
  call dealloc(v)
...
subroutine reduce_prv(v, length)
  ...
  if (.not. allocated(v)) then !v not allocated
    call info('(module_ALLOC_real_DOUBLE_r1/reduce) Variable not allocated'); return
  end if
  if (length > size(v, 1)) then !p too large
    call info('(module_ALLOC_real_DOUBLE_r1/reduce) Given dimension is too large'); return
  end if    
...
!-----------------------------------------------------------------------
subroutine set_prv(v, val, row, fit_row, add)
    
  real(DOUBLE), dimension(:), allocatable :: v
  real(DOUBLE), intent(in)                :: val
  integer, intent(in), optional   :: row
  logical, intent(in), optional   :: fit_row, add
  integer :: r
  real(DOUBLE) :: preval
  
  r = enlarge1(v, row, fit_row) !extension in rows, if necessary
  preval = 0. !previous value
  if (present(add)) then; if (add) preval = v(r); end if
  v(r) = preval + val 
