module module_ALLOC_real_alloc_r2
!-----------------------------------------------------------------------
...
!Types
type real_alloc_r1 !every row is a rank 1 allocatable array
  real, dimension(:), allocatable :: col
end type

type real_alloc_r2 !derived data type is composed of rows
  type(real_alloc_r1), dimension(:), allocatable :: row
end type
...
subroutine dealloc_prv(v)
  ...
  if (res /= 0) call error('(module_ALLOC_real_alloc_r2/dealloc) Unable to deallocate variable')
...
subroutine alloc_prv(v, rows)
  ...
  if (res /= 0) call error('(module_ALLOC_real_alloc_r2/alloc) Unable to allocate variable')
  do i = 1, n
...
subroutine reduce_prv(v, rows)
  ...
  type(real_alloc_r2) :: v, temp
  integer, intent(in) :: rows

  if (.not. allocated(v%row)) then
    call info('(module_ALLOC_real_alloc_r2/reduce) Variable not allocated'); return
  end if
  if (rows > size(v%row, 1)) then
    call info('(module_ALLOC_real_alloc_r2/reduce) Given dimension is too large'); return
  end if    
...
function enlarge_rows(v, row, fit) result(p)
  ...
  if (present(fit)) then 
    ...
    else; do while (n < p); call extend(v); n = size(v%row, 1); end do !fit is false
    end if
  else; do while (n < p); call extend(v); n = size(v%row, 1); end do   !fit is false
  end if
...
function enlarge_cols(v, r, col, fit) result(p)
  ...
  !ALLOCATE
  if (present(fit)) then 
    if (fit) then; call extend(v%row(r)%col, extension=p-n)   !fit is true
    else; do while (n < p); call extend(v%row(r)%col); n = size(v%row(r)%col, 1); end do !fit is false
    end if
  else; do while (n < p); call extend(v%row(r)%col); n = size(v%row(r)%col, 1); end do   !fit is false
  end if
...
!-----------------------------------------------------------------------
! transf: transfer data between to real_alloc_r2 objects
!-----------------------------------------------------------------------
subroutine transf(u, v)
  
  type(real_alloc_r2) :: u, v
  integer :: i
