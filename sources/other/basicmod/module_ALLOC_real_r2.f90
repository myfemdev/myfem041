module module_ALLOC_real_r2
!-----------------------------------------------------------------------
! Module for memory allocation of real rank 2 allocatable arrays
...
subroutine dealloc_prv(v)
  ...
  if (res /= 0) call error('(module_ALLOC_real_r2/dealloc) Unable to deallocate variable')
...
subroutine alloc_prv(v, rows, cols)
  ...
  if (res /= 0) call error('(module_alloc_real_r2/alloc) unable to allocate variable')
  v(1:n, 1:m) = 0.
...
subroutine reduce_prv(v, rows, cols)
  ...
  if (.not. allocated(v)) then !v not allocated
    call info('(module_ALLOC_real_r2/reduce) Variable not allocated'); return
  end if
  ...
  if (rows >  size(v,1) .or.  cols >  size(v,2)) then !rows or cols too large
    call info('(module_ALLOC_real_r2/reduce) Given dimension is too large'); return
  end if    
...
subroutine setv_prv(v, val, row, col, fit_row, fit_col, add)
  ...
  integer, intent(in), optional     :: row, col
  logical, intent(in), optional     :: fit_row, fit_col, add
  real, dimension(size(val,1)) :: preval
  integer :: r, c
    
  if (present(row) .eqv. present(col)) call error('(module_ALLOC_real_r2/set) &
