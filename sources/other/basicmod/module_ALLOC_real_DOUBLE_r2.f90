module module_ALLOC_real_DOUBLE_r2
!-----------------------------------------------------------------------
! Module for memory allocation of real DOUBLE rank 2 allocatable arrays
...
!-----------------------------------------------------------------------
use module_COMPILER_DEPENDANT, only: DOUBLE
...
subroutine dealloc_prv(v)
  ...
  if (res /= 0) call error('(module_ALLOC_real_DOUBLE_r2/dealloc) Unable to deallocate variable')
...
subroutine alloc_prv(v, rows, cols)
  ...
  m = DEFAULT_ALLOC; if (present(cols)) m = cols
  if (allocated(v)) then
  ...
  if (res /= 0) call error('(module_alloc_real_DOUBLE_r2/alloc) unable to allocate variable')
  v(1:n, 1:m) = 0.
...
subroutine extend_prv(v, dim, extension)
  ...
  real(DOUBLE), dimension(:,:), allocatable :: v, temp
  ...
  temp(1:n0, 1:m0) = v
  temp(n0+1:n, 1:m0) = 0.  !reset new positions
  temp(1:n0, m0+1:m) = 0.  !reset new positions
  call dealloc(v)
...
subroutine reduce_prv(v, rows, cols)
  ...
  if (.not. allocated(v)) then !v not allocated
    call info('(module_ALLOC_real_DOUBLE_r2/reduce) Variable not allocated'); return
  end if
  ...
  if (rows > size(v,1) .or. cols > size(v,2)) then !rows or cols too large
    call info('(module_ALLOC_real_DOUBLE_r2/reduce) Given dimension is too large'); return
  end if
...
subroutine setv_prv(v, val, row, col, fit_row, fit_col, add)
  ...
  real(DOUBLE), dimension(:,:), allocatable :: v
  real(DOUBLE), dimension(:), intent(in)    :: val
  integer, intent(in), optional             :: row, col
  logical, intent(in), optional             :: fit_row, fit_col, add
  real(DOUBLE), dimension(size(val,1)) :: preval
  integer :: r, c

  if (present(row) .eqv. present(col)) call error('(module_ALLOC_real_DOUBLE_r2/set) &
