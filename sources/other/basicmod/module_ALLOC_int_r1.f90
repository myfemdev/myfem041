...
!-----------------------------------------------------------------------
! Module for memory allocation of integer rank 1 allocatable arrays
...
subroutine dealloc_prv(v)
  ...
  if (res /= 0) call error('(module_ALLOC_int_r1/dealloc) Unable to deallocate variable')
...
subroutine alloc_prv(v, rows)
  ...
  if (res /= 0) call error('(module_alloc_int_r1/alloc) unable to allocate variable')
  v(1:n) = 0
...
subroutine extend_prv(v, extension)
  ...
  integer, dimension(:), allocatable :: v, temp
...
subroutine reduce_prv(v, length)
  ...
  if (length > size(v, 1)) then !p too large
    call info('(module_ALLOC_int_r1/reduce) Given dimension is too large'); return
  end if    
...
!-----------------------------------------------------------------------
subroutine set_prv(v, val, row, fit_row, add, insert)
    
  integer, dimension(:), allocatable :: v
  integer, intent(in)                :: val
  integer, intent(in), optional      :: row
  logical, intent(in), optional      :: fit_row, add, insert
  integer :: r
  integer :: preval
  
  if (present(insert)) then
    r = enlarge1(v, size(v,1)+1, fit_row)
  else
    r = enlarge1(v, row, fit_row) !extension in rows, if necessary
  end if  
  preval = 0 !previous value
  if (present(add)) then; if (add) preval = v(r); end if
  if (present(insert)) v((row+1):size(v,1)) = v(row:(size(v,1)-1))
  v(row) = preval + val 
